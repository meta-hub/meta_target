...
local function ensureBind(checkedControls,bind,key,disabledKey,fn,disabledFn,onComplete)
  checkedControls[bind.control] = checkedControls[bind.control] or {}

  if not bind.onDisabled then
    if checkedControls[bind.control][key] == nil then
      checkedControls[bind.control][key] = fn(0,bind.control)
    end

    local res = checkedControls[bind.control][key]
    ...
    if checkedControls[bind.control][disabledKey] == nil then
      checkedControls[bind.control][disabledKey] = disabledFn(0,bind.control)
    end

    local res = checkedControls[bind.control][disabledKey]
...
local callbackHandlers = {
  onPressed = function(checkedControls,bind)
    return ensureBind(checkedControls,bind,'justPressed', 'disabledJustPressed', IsControlJustPressed, IsDisabledControlJustPressed, bind.callbacks.onPressed)
  end,

  onReleased = function(checkedControls,bind)
    return ensureBind(checkedControls,bind,'justReleased','disabledJustReleased',IsControlJustReleased,IsDisabledControlJustReleased,bind.callbacks.onReleased)
  end,

  onHeld = function(checkedControls,bind)
    return ensureBind(checkedControls,bind,'held','disabledHeld',IsControlPressed,IsDisabledControlPressed,bind.callbacks.onHeld)
  end
}

local function checkBind(checkedControls,bind)
  if bind.enabled then
    for name,fn in pairs(bind.callbacks) do
      callbackHandlers[name](checkedControls,bind)
    end
  end
end
...
Citizen.CreateThread(function()
  ...
  while true do
    local checkedControls = {}

    for i=1,#binds do
      local bind = binds[i]
      checkBind(checkedControls,bind)
    end
...
local function newBindId()
  local id = GetRandomIntInRange(0,100000)

  while bindIds[id] do
    id = GetRandomIntInRange(0,100000)
  end
...
for label,hash in pairs(controlHashList) do
  local unsigned = hash     % 0x100000000
  local signed   = unsigned - 0x100000000

  controls[unsigned] = label
  controls[signed]   = label

  local lower = label:lower()
  local upper = label:upper()

  controls[lower] = unsigned
  controls[upper] = unsigned
  controls[label] = unsigned
end
...
Controls.Bind  = function(...)
  ...
  for i=1,select("#",...) do
    local ref = select(i,...)
    local bindId = newBindId()
    local controlHash = type(ref.control) == 'number' and ref.control or controlHashList[ref.control]

    local bind = {
      enabled = true,
      bindId = bindId,
      control = controlHash,
      onDisabled = ref.onDisabled,
      callbacks = {
        onPressed = ref.onPressed,
        onReleased = ref.onReleased,
        onHeld = ref.onHeld
      },
...
Controls.Remove = function(...)
  for i=1,select("#",...) do
    local bindId = select(i,...)

    for j=#binds,-1,1 do
      if binds[j].bindId == bindId then
        table.remove(binds,j)
      end
    end
  end
end
